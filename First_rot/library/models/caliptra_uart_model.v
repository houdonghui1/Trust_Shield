...
`ifdef UART_MONITOR
	$display($time, "[CA_UART MONITOR] tx data %10h.", tx_data);
`endif
endtask
...
`ifdef UART_MONITOR
	$display($time, "[CA_UART MONITOR] tx data %10h with %2d stop bit.", tx_data, stop_bit_num);
`endif
endtask
...
`ifdef UART_MONITOR
	$display($time, "[CA_UART MONITOR] tx data %10h with %1b parity bit & %2d stop bit.", tx_data, tx_parity, stop_bit_num);
`endif
endtask
...
`ifdef UART_MONITOR
	$display($time, "[CA_UART MONITOR] rx data %10c with %2d stop bit.", rx_data, stop_bit_num);
`endif
endtask
...
`ifdef UART_MONITOR
	$display($time, "[CA_UART MONITOR] rx data %10h with %1b parity bit & %2d stop bit.", rx_data, rx_parity, stop_bit_num);
`endif
endtask
...
`ifdef UART_MONITOR
	$display($time, "[CA_UART MONITOR] tx noise.");
`endif
endtask
