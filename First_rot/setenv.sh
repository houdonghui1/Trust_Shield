...
export PRJ_DIR=/home/houdonghui/cm3_ahbmtx_mcu
...
export PRJ_CALIPTRA_WORKSPACE=${PRJ_DIR}/user/verilog/Caliptra
export PRJ_CALIPTRA_ROOT=${PRJ_DIR}/user/verilog/Caliptra/caliptra-rtl
export PRJ_RUN=${PRJ_CALIPTRA_WORKSPACE}/run
export PRJ_CA_SCRIPTS=${PRJ_CALIPTRA_ROOT}/tools/scripts
...
alias cdca='cd ${PRJ_CALIPTRA_ROOT}' 
alias cdr='cd ${PRJ_RUN}' 
...
function run_caliptra()
{
	cd ${PRJ_RUN}
	make -C ${PRJ_RUN} -f ${PRJ_CA_SCRIPTS}/Makefile TESTNAME=$1 vcs
	cd -
}

function run_caliptra_fsdb()
{
	cd ${PRJ_RUN}
	make -C ${PRJ_RUN} -f ${PRJ_CA_SCRIPTS}/Makefile TESTNAME=$1 vcs_and_fsdb
	cd -
}
