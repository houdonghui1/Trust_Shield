...
begin	
	...
	#50000
	u_sim_monitor.sim_monitor_init("CALIPTRA TOP REG WR start");
	u_swd_model.swd_init;
	for(int i = 0; i < 32'h10; i = i + 4)
	begin: AHB_WR_TEST
		u_swd_model.swd_writereg32(32'h30020000 + i, i);
		u_swd_model.swd_readreg32(32'h30020000 + i, rdata);
		u_swd_model.swd_readreg32(32'h30020000 + i, rdata);
		u_sim_monitor.sim_monitor_check(32'h30020000 + i, rdata, i);
	end

	#25000
	u_sim_monitor.sim_monitor_result;
	u_swd_model.swd_deinit;
	u_sim_monitor.sim_monitor_deinit("CALIPTRA TOP REG WR start");

/*

	#50000
	u_sim_monitor.sim_monitor_init("CALIPTRA TOP REG WR end");
	u_swd_model.swd_init;
	for(int i = 0; i < 32'h10; i = i + 4)
	begin: APB0_WR_TEST
