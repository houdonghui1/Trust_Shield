...
`PRJ_CALIPTRA_ROOT`: Defines the absolute path to the Project repository root (called "Caliptra" or "caliptra-rtl"). Recommended to define as `${CALIPTRA_WORKSPACE}/Caliptra`.<BR>
...
VF files provide absolute filepaths (prefixed by the `PRJ_CALIPTRA_ROOT` environment variable) to each compile target for the associated component.<BR>
...
5. Invoke `${PRJ_CALIPTRA_ROOT}/tools/scripts/Makefile` with target 'program.hex' to produce SRAM initialization files from the firmware found in `src/integration/test_suites/${TESTNAME}`
    - E.g.: `make -f ${PRJ_CALIPTRA_ROOT}/tools/scripts/Makefile program.hex`
    - NOTE: TESTNAME may also be overridden in the makefile command line invocation, e.g. `make -f ${PRJ_CALIPTRA_ROOT}/tools/scripts/Makefile TESTNAME=iccm_lock program.hex`
...
5. Run Caliptra/tools/scripts/Makefile, which provides steps to run a top-level simulation in Verilator
    - Example command:
        `make -C <path/to/run/folder> -f ${PRJ_CALIPTRA_ROOT}/tools/scripts/Makefile TESTNAME=${TESTNAME} debug=1 verilator`
...
1. Compile the validation firmware (as described in [Regression Tests](#Regression-Tests)) that will run on Caliptra's embedded RISC-V core
    ...
    - `make -f ${PRJ_CALIPTRA_ROOT}/tools/scripts/Makefile TESTNAME=caliptra_top program.hex`
    - `make -f ${PRJ_CALIPTRA_ROOT}/tools/scripts/Makefile TESTNAME=caliptra_fmc caliptra_fmc.hex`
    - `make -f ${PRJ_CALIPTRA_ROOT}/tools/scripts/Makefile TESTNAME=caliptra_rt  caliptra_rt.hex`
