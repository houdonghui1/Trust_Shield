...
def verilateTB(scratch):
    ...
    mfile = os.path.join(os.environ.get('PRJ_CALIPTRA_ROOT'),"tools/scripts/Makefile")
...
def getTestNames():
    l0_regress_file = os.path.join(os.environ.get('PRJ_CALIPTRA_ROOT'), "src/integration/stimulus/L0_regression.yml")
    testPaths = []
    x = ''
...
def main():
    # Env vars $CALIPTRA_WORKSPACE and $PRJ_CALIPTRA_ROOT must be set/present
    if (os.environ.get('CALIPTRA_WORKSPACE') is None):
        logger.error("CALIPTRA_WORKSPACE not defined!")
        return 1
    if (os.environ.get('PRJ_CALIPTRA_ROOT') is None):
        logger.error("PRJ_CALIPTRA_ROOT not defined!")
        return 1
    elif ((os.environ.get('PRJ_CALIPTRA_ROOT') != os.path.join(os.environ.get('CALIPTRA_WORKSPACE'), "Caliptra"    )) and
          (os.environ.get('PRJ_CALIPTRA_ROOT') != os.path.join(os.environ.get('CALIPTRA_WORKSPACE'), "caliptra-rtl"))):
        logger.error(f"PRJ_CALIPTRA_ROOT definition [{os.environ.get('PRJ_CALIPTRA_ROOT')}] is invalid! Expected [{os.path.join(os.environ.get('CALIPTRA_WORKSPACE', 'Caliptra'))}] or [{os.path.join(os.environ.get('CALIPTRA_WORKSPACE', 'caliptra-rtl'))}]")
        return 1
