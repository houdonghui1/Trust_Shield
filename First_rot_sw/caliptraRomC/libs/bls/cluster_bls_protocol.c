...
#ifdef CLUSTER_BLS_FIXED_L1_DEMO_KEY
/*
 * 48KiB L1 demo profile.  Enrollment and certificate decoding accept only
 * this pre-provisioned registration.  The external verifier still validates
 * the PoP and the final aggregate; production builds must not use this path.
 */
...
static bool demo_l1_registration_matches(
    const cluster_bls_registration_t *registration) {
  return memcmp(registration->key_id, kDemoL1KeyId,
                kClusterBlsKeyIdBytes) == 0 &&
         memcmp(registration->public_key, kDemoL1PublicKey,
                kOtBlsPublicKeyBytes) == 0 &&
         memcmp(registration->proof_of_possession, kDemoL1Proof,
...
#ifdef CLUSTER_BLS_FIXED_L1_DEMO_KEY
  return demo_l1_registration_matches(registration);
#else
