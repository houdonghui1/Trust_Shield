...
void soc_ifc_clear_execute_reg() {
    printf("SOC_IFC: Clear execute reg");
    uint32_t reg;
...
uint8_t soc_ifc_chk_execute_uc() {
    ...
    printf("SOC_IFC: Check mbox_status.mbox_fsm_ps\n");
    ...
    if (state == MBOX_EXECUTE_UC) {
        printf("SOC_IFC: Check mbox_status.mbox_fsm_ps found MBOX_EXECUTE_UC\n");
        return 0;
    } else if (state == MBOX_IDLE) {
        printf("SOC_IFC: Check mbox_status.mbox_fsm_ps found MBOX_IDLE\n");
        return 1;
    } else if (state == MBOX_ERROR) {
        printf("SOC_IFC: Check mbox_status.mbox_fsm_ps found MBOX_ERROR, mailbox force-unlock needed\n");
        return 0xF;
    } else {
        printf("SOC_IFC: Check mbox_status.mbox_fsm_ps found unexpected state 0x%x\n", state);
        return 0xFF;
    }
}

void soc_ifc_set_mbox_status_field(enum mbox_status_e field) {
    printf("SOC_IFC: Set mbox_status field: 0x%08x\n", (unsigned int)field);
    uint32_t reg;
...
void soc_ifc_set_flow_status_field(uint32_t field) {
    printf("SOC_IFC: Set flow_status field: 0x%08x\n", (unsigned int)field);
    uint32_t reg;
...
void soc_ifc_w1clr_sha_lock_field(uint32_t field) {
    printf("SOC_IFC: Clear SHA accelerator lock by writing '1: 0x%08x\n", (unsigned int)field);
    uint32_t reg;
    reg = lsu_read_32(CLP_SHA512_ACC_CSR_LOCK);
    if (field & ~SHA512_ACC_CSR_LOCK_LOCK_MASK) {
       printf("SOC_IFC: Bad field value");
    } 
...
void soc_ifc_clr_flow_status_field(uint32_t field) {
    printf("SOC_IFC: Clear flow_status field: 0x%08x\n", (unsigned int)field);
    uint32_t reg;
    reg = lsu_read_32(CLP_SOC_IFC_REG_CPTRA_FLOW_STATUS);
...
mbox_op_s soc_ifc_read_mbox_cmd() {
    ...
    //printf("SOC_IFC: CMD from mailbox: 0x%x\n", op.cmd);

    //read mbox dlen
    op.dlen = lsu_read_32(CLP_MBOX_CSR_MBOX_DLEN);
    //printf("SOC_IFC: DLEN from mailbox: 0x%08x\n", (unsigned int)op.dlen);
...
void soc_ifc_mbox_fw_flow(mbox_op_s op) {
    ...
    printf("SOC_IFC: Beginning mbox fw flow\n");
    if (op.dlen > MBOX_DIR_SPAN) {
        printf("FATAL: Invalid dlen passed to mbox fw flow: 0x%08x\n", (unsigned int)op.dlen);
    ...
    if (op.cmd == MBOX_CMD_FMC_UPDATE) {
        ...
        printf("FATAL: Invalid cmd passed to mbox fw flow: 0x%x\n", op.cmd);
    ...
    if (iccm_cp_size == 0x0 || (iccm_cp_size + 0x20) > op.dlen) { // 0x20 fudge factor for linker offsets that contain iccm/dccm copy size
        printf("Found invalid iccm size in firmware image received from SOC! Max expected 0x%08x, got 0x%08x\n", (unsigned int)op.dlen, (unsigned int)iccm_cp_size);
    ...
    if (dccm_cp_size == 0x0 || ((iccm_cp_size<<2) + dccm_cp_size + 0x20) > op.dlen) { // 0x20 fudge factor for linker offsets that contain iccm/dccm copy size
        printf("Found invalid dccm size in firmware image received from SOC! Max expected 0x%08x, got 0x%08x\n", (unsigned int)(op.dlen - iccm_cp_size - 0x20), (unsigned int)dccm_cp_size);
...
void soc_ifc_fw_update(mbox_op_s op) {
    ...
    printf( "Copying code from mailbox to ICCM\n");
