...
int read_and_compare(uint32_t addr, uint32_t exp_data) {
  ...
  if (act_data != exp_data) {
    printf("Got:%08x Want: %08x", (unsigned int)act_data, (unsigned int)exp_data);
    return 1;
  }
  return 0;
}
...
/*   printf("  - Polling addr=%08x until it reads back %08x...\n", (unsigned int)addr,
...
void end_sim_if_itrng_disabled() {
  ...
  if (hw_cfg & SOC_IFC_REG_CPTRA_HW_CONFIG_ITRNG_EN_MASK) {
    //printf("Internal TRNG is enabled, running TRNG smoke test\n");
  } else {
    //printf("Internal TRNG is not enabled, skipping TRNG smoke test\n");
    while (1)
      ;
  }
}
...
int generate_random_numbers(int num_randoms, uint8_t *output) {
    ...
    for (int i = 0; i < num_randoms; i++) {
        if (i % 4 == 0) {
            lsu_write_32(CLP_CSRNG_REG_CMD_REQ, 0x4903);
            poll_reg(CLP_CSRNG_REG_GENBITS_VLD, CSRNG_REG_GENBITS_VLD_GENBITS_VLD_MASK);
            uint32_t word = lsu_read_32(CLP_CSRNG_REG_GENBITS);
            output[i]   = (word >> 0)  & 0xFF;
            output[i+1] = (word >> 8)  & 0xFF;
            output[i+2] = (word >> 16) & 0xFF;
            output[i+3] = (word >> 24) & 0xFF;
        }
    }
    return 0;
}