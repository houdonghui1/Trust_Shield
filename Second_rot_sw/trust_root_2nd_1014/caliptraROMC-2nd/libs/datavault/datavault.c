...
__attribute__((section(".dccm"))) widereg_t dv_regs [DV_PFX_COUNT] = {
...
widereg_t *find_dv_reg_by_pfx(const char * c) {
...
int store_to_datavault(uint32_t *datax, uint32_t *datay, uint32_t regx, uint32_t regy) {
    volatile uint32_t * reg_ptr;
    uint32_t offset;
    uint32_t ctrl_regx_addr = CLP_DV_REG_DATAVAULTCTRL_0 + (4 * regx);
    uint32_t ctrl_regy_addr = CLP_DV_REG_DATAVAULTCTRL_0 + (4 * regy);
    printf("func: %s, line: %d\n", __func__, __LINE__);
    if (regx >= 10 || regy >= 10) {
        printf("Error: Register index out of range (0-9)\n");
        return -1;
    }
    
    reg_ptr = (uint32_t*)CLP_DV_REG_DATA_VAULT_ENTRY_0_0 + (12 * regx);
    offset = 0;
    while (offset < 12) {
        *reg_ptr++ = datax[offset++];
    }
    lsu_write_32(ctrl_regx_addr, 0x1);

    reg_ptr = (uint32_t*)CLP_DV_REG_DATA_VAULT_ENTRY_0_0 + (12 * regy);
    offset = 0;
    while (offset < 12) {
        *reg_ptr++ = datay[offset++];
    }
    lsu_write_32(ctrl_regy_addr, 0x1);
    
    printf("Success: Data stored to DATA_VAULT_ENTRY_%08x and DATA_VAULT_ENTRY_%08x\n", (unsigned int)regx, (unsigned int)regy);
    return 0;
}

int read_from_datavault(uint32_t *datax, uint32_t *datay, uint32_t regx, uint32_t regy) {
    volatile uint32_t * reg_ptr;
    uint32_t offset;
    printf("func: %s, line: %d\n", __func__, __LINE__);
    if (regx >= 10 || regy >= 10) {
    ...
    offset = 0;
    while (offset < 12) {
        datax[offset++] = *reg_ptr++;
    }

    reg_ptr = (uint32_t*)CLP_DV_REG_DATA_VAULT_ENTRY_0_0 + (12 * regy);
    offset = 0;
    while (offset < 12) {
        datay[offset++] = *reg_ptr++;
    }

    printf("Success: Data read from DATA_VAULT_ENTRY_%08x and DATA_VAULT_ENTRY_%08x\n", (unsigned int)regx, (unsigned int)regy);
    return 0;
}