...
void wait_for_sha256_intr(){
    ...
    printf("Received SHA256 notif intr with status = %ld\n", cptra_intr_rcv.sha256_notif);
}
...
void sha256_flow(sha256_io block, uint8_t mode, sha256_io digest, uint8_t pcr_entry){
    ...
    if(digest.data_size == 0){
        ...
        printf("Storing SHA256 digest to PCR entry %d\n", pcr_entry);
        reg_ptr = (uint32_t *)(CLP_PV_REG_PCR_ENTRY_0_0 + (pcr_entry * 12 * 4)); // PCR entry base address
        offset = 0;
        while (offset < 8) {
            *reg_ptr++ = sha256_digest[offset++];
        }
    } else {
        while (reg_ptr <= (uint32_t*) CLP_SHA256_REG_SHA256_DIGEST_7) {
            sha256_digest[offset] = *reg_ptr;
...
void sha256_flow_produce(sha256_io block, uint8_t mode, sha256_io digest){
    volatile uint32_t * reg_ptr;
    uint8_t offset;
