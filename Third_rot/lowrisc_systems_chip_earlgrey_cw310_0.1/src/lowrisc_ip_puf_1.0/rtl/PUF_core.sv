...
module PUF_core (
    ...
    (*mark_debug = "true"*)output response_valid,
    output [`WIDTH - 1:0] response,
    output response_valid_re,
    output [`WIDTH/32 -1:0]response_re,
    output rng4bits_done_re,
    (*mark_debug = "true"*)output rng4bits_done,
    (*mark_debug = "true"*)output [3:0] rng4bits/*synthesis syn_keep=1*/
);
...
(*keep_hierarchy = "true"*) response_rng4bits u_response_rng4bits(
  .clk                (clk                ),
  .rst                (rst_n                ),
  .response_done_2bit (response_done_2bit_puf ),
  .response2bit       (response2bit_puf       ),
