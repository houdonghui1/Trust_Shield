...
void set_timer(uint32_t tick_hz) {
  ...
  LOG_INFO("timer_counter = %08x",&timer_counter);
  state = dif_rv_timer_counter_write(&timer, kHart, timer_counter);

  state = dif_rv_timer_counter_read(&timer, kHart, &timer_counter);
...
bool test_main(void) {
  ...
  //state = dif_rv_timer_counter_set_enabled(&timer, kHart, kDifToggleEnabled);
  const uint64_t elapsed_millis1 = udiv64_slow(tick1 * 1000, 1000000, NULL);

  LOG_INFO("elapsed_millis1 = %u",elapsed_millis1);

  for (size_t i = 0; i < 5000; i++)
  {
    SM3_hash_function(message_600word,600,full,result_buf1);
  ...
  //state = dif_rv_timer_counter_set_enabled(&timer, kHart, kDifToggleEnabled);
  const uint64_t elapsed_millis2 = udiv64_slow(tick2 * 1000, 1000000, NULL);

  LOG_INFO("elapsed_millis2 = %u",elapsed_millis2);
...
bool test_main(void) {
    ...
    LOG_INFO("%08x%08x%08x%08x%08x%08x%08x%08x",result_buf1[0],result_buf1[1],result_buf1[2],result_buf1[3],result_buf1[4],result_buf1[5],result_buf1[6],result_buf1[7]);
    LOG_INFO("The process takes 1.8us ");
    LOG_INFO(" ");
    LOG_INFO("-------------------");
    LOG_INFO("30 word message test");
    LOG_INFO("The expected outcome is: e18087a80c74aec4d2476214a9ef4cdb14ced4b6d19e98d9dc15e2c63f8e6dba");
    SM3_hash_function(message_30word,30,full,result_buf2);
    LOG_INFO("The calculated result is:");
    LOG_INFO("%08x%08x%08x%08x%08x%08x%08x%08x",result_buf2[0],result_buf2[1],result_buf2[2],result_buf2[3],result_buf2[4],result_buf2[5],result_buf2[6],result_buf2[7]);
    LOG_INFO("The process takes 0.9us");
    LOG_INFO(" ");
    LOG_INFO("*******************");
    return true;
}
