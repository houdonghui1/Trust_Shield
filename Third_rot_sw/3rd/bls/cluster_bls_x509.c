...
bool cluster_bls_x509_append_binding(
  ...
  uint8_t rebuilt[kClusterBlsMaxTbsBytes];
  ...
  if (new_wrapper_len == 1 || new_tbs_len == 1 || new_tbs_len > sizeof(rebuilt) ||
      new_tbs_len > *out_len) return false;

  rebuild_cursor = rebuilt;
  rebuild_end = rebuilt + sizeof(rebuilt);
  ...
  *rebuild_cursor++ = kDerSequence;
  ...
  if ((size_t)(rebuild_cursor - rebuilt) != new_tbs_len) return false;
  memcpy(out, rebuilt, new_tbs_len);
  *out_len = new_tbs_len;
  return true;
}
