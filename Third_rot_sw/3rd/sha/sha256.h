...
struct HASH_CTX_256;  // forward decl

typedef struct HASH_VTAB_256 {
  void (* const init)(struct HASH_CTX_256*);
  void (* const update)(struct HASH_CTX_256*, const void*, size_t);
  const uint8_t* (* const final)(struct HASH_CTX_256*);
  const uint8_t* (* const hash)(const void*, size_t, uint8_t*);
  unsigned int size;
} HASH_VTAB_256;

typedef struct HASH_CTX_256 {
  const HASH_VTAB_256 * f;
  uint64_t count;
  uint8_t buf[64];
  uint32_t state[8];
} HASH_CTX_256;

typedef HASH_CTX_256 LITE_SHA256_CTX;
