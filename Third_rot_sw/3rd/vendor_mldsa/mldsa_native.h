...
#define MLDSA87_PK_SIZE  MLDSA87_PUBLICKEYBYTES  // 公钥长度（MLDSA87=2592）
#define MLDSA87_SK_SIZE  MLDSA87_SECRETKEYBYTES  // 私钥长度（MLDSA87=4896）
#define MLDSA87_SIG_SIZE MLDSA87_BYTES           // 签名长度（MLDSA87=4627）
...
int PQCP_MLDSA_NATIVE_MLDSA87_keypair(uint8_t *pk, uint8_t *sk);

// 签名函数
MLD_API_MUST_CHECK_RETURN_VALUE
int PQCP_MLDSA_NATIVE_MLDSA87_sign(uint8_t *sig, const uint8_t *msg, size_t msg_len, const uint8_t *sk);

// 验签函数
MLD_API_MUST_CHECK_RETURN_VALUE
int PQCP_MLDSA_NATIVE_MLDSA87_verify(const uint8_t *sig, const uint8_t *msg, size_t msg_len, const uint8_t *pk);
