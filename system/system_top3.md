...
| 4 | 三级可信根 | 将控制权交给二级可信根，二级验证一级可信根，结果返回三级 | 二级可信根 | 层级化验证：二级校验一级PCR与证书 |
| 5 | 三级可信根 | 组成Quote包（AK证书 + PCR值 + Nonce），用ML-DSA(AK)私钥签名 | — | 后量子签名抗量子攻击 |
| 6 | 三级可信根 | 将Quote包发送至验证方 | 验证方 | — |
| 7 | 验证方 | 用CA证书验证AK证书，用AK公钥验证签名，检查Nonce和PCR值 | — | 完整性校验 + 防重放 + 度量值判定 |
| 8 | 验证方 | 判定设备可信 | — | 可信则放行，不可信则告警 |

**层级化验证机制**：远程证明采用与信任链建立一致的层级化验证模式——三级可信根验证二级可信根的度量值与证书签名，二级可信根验证一级可信根的度量值与证书签名，验证结果逐级返回三级可信根聚合。三级可信根最终将完整的度量链信息（一级PCR→二级PCR→三级PCR）打包进Quote包，实现"从芯片到集群"的全链路可信证明。
...
| 二级↔三级 | 物理串口线缆 | UART（115200-8N1）+ 32B分块协议 | 度量请求信号、caliptraROM.bin、启动信号、TBS证书、设备证书、PCR扩展请求与响应 | ACK+超时重传，帧头帧尾边界识别 |
| 三级↔CA | 串口（经SAM3X） | 自定义证书签发协议 | EK证书、AK公钥、AK证书 | 证书链验证 + ML-DSA公钥扩展字段 |
